...
HANDLE createNewProcess(int procNumber) {
	...
	TCHAR str[] = TEXT("C:\\Users\\abutg\\source\\repos\\lab2SpovmChild\\x64\\Debug\\lab2SpovmChild.exe");
	TCHAR* command = new TCHAR[commandArgs.length() + 1];
	mbstowcs_s(NULL, command, _tcslen(command), commandArgs.c_str(), commandArgs.length());
	if (!CreateProcess(str,//âûçâàòü âòîðîé ïðîåêò ñ äî÷åðíèì ïðîöåññîì
		command,// ïåðåäàòü àðãóìåíòû êîìàíäíîé ñòðîêè
		NULL,
		NULL,
		FALSE,
		0,
